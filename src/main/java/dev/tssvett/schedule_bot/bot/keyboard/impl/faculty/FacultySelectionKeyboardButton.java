...
import static dev.tssvett.schedule_bot.bot.enums.keyboard.Action.COURSE_CHOOSE;
...
public class FacultySelectionKeyboardButton implements KeyboardButton {
    ...
    private SendMessage sendCourseSelectionMessage(long userId, long chatId) {
        return SendMessage.builder()
                .chatId(chatId)
                .text(MessageTextConstants.REGISTER_CHOOSE_COURSE_MESSAGE)
    ...
    private SendMessage sendAlreadySelectedFacultyMessage(long chatId) {
        return SendMessage.builder()
                .chatId(chatId)
                .text(MessageTextConstants.FACULTY_CLICK_WITH_ERROR_STATE)
                .build();
    }
}
