...
class TodayScheduleBotCommandTest {
    @Mock
    ...
    @InjectMocks
    TodayScheduleBotCommand todayScheduleBotCommand;
    ...
    void execute_happyPath() {
        ...
        when(dateUtils.calculateCurrentDayName()).thenReturn("понедельник");
        when(scheduleStringFormatter.formatDay(map, "понедельник"))
                .thenReturn("StringLessongs");

        //Act
        SendMessage result = todayScheduleBotCommand.execute(1L, 1L);

        //Assert
        verify(lessonService).getWeekScheduleMapByDate(1L);
        verify(dateUtils).calculateCurrentDayName();
